...
client.on('ready', () => {
	...
	while (true) {
		...
		var query = con.query(`SELECT * FROM remindme WHERE new_date < ${now}`);
		...
		query.on("result", row => {
			var remindText	=	`⌚ You requested a reminder on ${row.origin_date.format("dddd, MMMM Do YYYY, h:mm:ss a ZZ")} to remind you:\n`+
								`\`${row.note}\``;
			var remindFoot	=	`This reminder was due ${row.new_date.format("dddd, MMMM Do YYYY, h:mm:ss a ZZ")}, and delivered `+
								`${now.format("dddd, MMMM Do YYYY, h:mm:ss a ZZ")}. If this was not on time, please message ZedFish#2430.`;
			var colour		= 0x33b23b;
			var embed		= {
				color:			colour,
		...
		console.log("Waiting");
		var currTime = moment();
		var endTime = currTime.add(10, "seconds");
