...
bot.on("guildMemberRemove", member => {
    ...
    var chairEmote      = server.emojis.get("445099687402536960")
    if (member.roles.has(confirmedRole)) {
        try {
            bot.sendWelcome(null, `<:x:364941928070119424> **${member.displayName}** just retired from **r/AFL**. Please celebrate as we chair **${member.displayName}** off.\n${chairEmote}`, "error", offTopicChannel);
        } catch (e) {
            console.log(`SendWelcome Error:\n${e}\nMember: ${member.displayName}\nChannel: ${offTopicChannel}`)
        }
    } else {
        try{
            bot.sendWelcome(null, `<:x:364941928070119424> **${member.displayName}** just retired from **r/AFL**. Please celebrate as we chair **${member.displayName}** off.\n${chairEmote}`, "error", welcomeChannel);
        } catch (e) {
            console.log(`SendWelcome Error:\n${e}\nMember: ${member.displayName}\nChannel: ${welcomeChannel}`)
        }
    }
