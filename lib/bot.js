...
var modError = function(msg) {
	...
	modChannel.send(`Hey guys, I'm having an issue in #welcome, can somebody take a look? They may have posted their Reddit username in a sentence where I can't reach it. <@${client.OWNERID}>`);
	return;
}
...
client.on("message", msg => {
	...
	if (msg.channel === welcomeChannel) {
		...
		if (redditUser) {
			redditAPI.getUser(redditUser[1]).created.then(time => {
				var d = new Date();
