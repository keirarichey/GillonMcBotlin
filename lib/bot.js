...
var welcome = function(msg, redditUser) {
	...
	msg.author.addRole(confirmedRole);

	var welcomeEmbedText	= `✅ Welcome <@${msg.author.id}> to the r/AFL Discord Server! Don't forget to check the #rules, and head over to #bot_spam to ` +
							  `grab a team flair or check match scores!`
	var welcomeEmbed		= {
	...
	userDumpChannel.send(`${msg.author.tag} is https://reddit.com/u/${redditUser}`);
	return;
}

var modError = function() {
	var modChannel = msg.guild.channels.get("271391673236455428");
	modChannel.send(`Hey guys, I'm having an issue in #welcome, can somebody take a look? <@${client.OWNERID}>`);
	return;
}
...
client.on("message", msg => {
	...
	if (msg.channel === welcomeChannel) {
		var confirmedRole	= "283573666351022081";
		if (msg.author.roles.has(confirmedRole)) {
			return;
		}

		var usernameRegex = /^(?:https?:\/*)?(?:reddit\.com)?(?:\/?u\/)?([^\s\.]+){1}\W*$/;

		var redditUser = msg.content.match(usernameRegex)[1];

		if (redditUser) {
			redditAPI.getUser(redditUser).created.then(time => {
				var d = new Date();
				var now = d.getTime(); // time in unix epoch milliseconds
				var twoWeeksAgo = now - 12096e5 //magic number: two weeks in ms
				if (time <= twoWeeksAgo) {
					welcome(msg, redditUser);
					return;
				} else {
