...
bot.sendWelcome = function (user, info, type, channel) {
    var icolor;
...
var loadCommands = function() {
    ...
    var scoreFiles = fs.readdirSync(__dirname+"/scores");
    for (let scoreFile of scoreFiles) {
        if (scoreFile.endsWith(".js")) {
            commands[scoreFile.slice(0, -3)] = require(__dirname+"/scores/"+scoreFile);
            if(bot.DETAILED_LOGGING) console.log("Loaded " + scoreFile);
        }
    }
...
var checkCommand = function(msg, isMention) {
    if(isMention) {
        ...
        try {
            ...
            bot.sendNotification(`❌ There was an error using this command.`, "error", msg)
            console.log(e);
        }
    } else {
...
bot.on("message", msg => {
    if (msg.content.length > 1) {
        if (msg.content.startsWith('<@'+bot.user.id+'>') || msg.content.startsWith('<@!'+bot.user.id+'>')) {
            try {
                if (msg.content.split(" ")[1].toLowerCase() in commands) {
            ...
            try {
                if (msg.content.split(bot.PREFIX)[1].split(" ")[0].toLowerCase() in commands) {
...
bot.on("guildMemberAdd", member => {
    ...
    try { 
    	bot.sendWelcome(member, `<:white_check_mark:364924816261906432> **${member.displayName}**, Welcome to r/AFL! Everything Australian Football, now on Discord! To access the main channels, you'll need to provide us a link to your reddit profile or your reddit name.\n\nThis is just to prevent spam, and we require your account is at least two weeks old.`, "success", welcomeChannel);
    } catch (e) {
...
bot.on("guildMemberRemove", member => {
    ...
    if (member.roles.has(confirmedRole)) {
        try {
            bot.sendWelcome(null, `<:x:364941928070119424> **${member.displayName}** just retired from **r/AFL**. Please celebrate as we chair **${member.displayName}** off.`, "error", offTopicChannel);
        } catch (e) {
            console.log(`SendWelcome Error:\n${e}\nMember: ${member.displayName}\nChannel: ${offTopicChannel}`)
        }
    }
    try{
        bot.sendWelcome(null, `<:x:364941928070119424> **${member.displayName}** just retired from **r/AFL**. Please celebrate as we chair **${member.displayName}** off.`, "error", welcomeChannel);
    } catch (e) {
        console.log(`SendWelcome Error:\n${e}\nMember: ${member.displayName}\nChannel: ${welcomeChannel}`)
    }
});
...
bot.on("guildMemberUpdate", (oldMember, newMember) => {
    ...
    if (!oldMember.roles.has(confirmedRole) && newMember.roles.has(confirmedRole)) {
        ...
        //bot.sendWelcome(null, `<:white_check_mark:364924816261906432> **${newMember.displayName}**, Welcome to r/AFL! Everything Australian Football, now on Discord!`, "success", offTopicChannel);
    }
});
