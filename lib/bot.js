...
bot.on("guildMemberAdd", member => {
    ...
    const welcomeChannel = SERVER.channels.find("id", "295589980733046784");
...
bot.on("guildMemberRemove", member => {
	const confirmedRole = "283573666351022081";
    const SERVER = member.guild;
    const welcomeChannel = SERVER.channels.find("id", "295589980733046784");
    const offTopicChannel = SERVER.channels.find("id", "193359073431912449");
...
bot.on("guildMemberUpdate", (oldMember, newMember) => {
	...
	if (!oldMember.roles.has(confirmedRole) && (newMember.roles.has(confirmedRole))) {
        bot.sendWelcome(`<:white_check_mark:364924816261906432> **${member.displayName}**, Welcome to r/AFL! Everything Australian Football, now on Discord!`, "success", offTopicChannel);
    }
});
