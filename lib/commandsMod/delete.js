module.exports = {
	...
	execute: async (msg, args) => {
		...
		if (!member.roles.has(modRole)) {
			...
			msg.channel.send({embed: permissionEmbed});
		}
		else {
			msg.channel.fetchMessages({limit: quant + 1})
				.then(messages => {
					msg.channel.bulkDelete(messages)
					.then(deletedMessages => {
						var removeText	= `❌ ${deletedMessages.size} messages have been removed by <@${member.id}>.`;
						var removeEmbed	= {
							color:			0x33b23b, // green
							description:	removeText
						};
						msg.channel.send({embed: removeEmbed});
					})
					.catch(e => {
						console.log(e)
					})
