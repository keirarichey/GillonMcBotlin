module.exports = {
    main: (bot, msg) => {
        ...
        var user = msg.member;
        if (!Object.values(bblTeamNames).includes(msg.content)) {
            ...
            bot.sendNotification(`<@${user.id}>: \<:duck:380975868383592450> OUT FOR A DUCK! \`${msg.content}\` is not an accepted input!\nPlease use "+help" or "<@${bot.user.id}> help" for help.`, "error", msg);
            return;
        } else {
            var drop_array = [];
            ...
            Array.from(Object.keys(bblTeams)).forEach(function(team_name) {
                if (bblTeams[team_name].includes(msg.content)) {
                        new_team = team_name;
                } else {
                    return;
                }
