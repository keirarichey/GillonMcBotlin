    ...
    main: (bot, msg) => {
        ...
        if (msg.channel == botChannel) {
            ...
            if (!Object.values(group_names).includes(msg.content)) {
                ...
                bot.sendNotification(`${user.displayName}: \<:bt:246541254182174720> THAT WAS OUT OF BOUNDS! \`${msg.content}\` is not an accepted input!\nPlease use "+help" or "<@${bot.user.id}> help" for help.`, "error", msg);
                return;
            } else {
                var role_name;
                var to_remove;
                ...
                if (user.roles.has(to_remove.id)) {
                    ...
                    bot.sendNotification(`<:negative_squared_cross_mark:364924937867231242> ${user.displayName} is no longer a part of the ${to_remove.name} group!`, "success", msg);
                } else {
                    bot.sendNotification(`<:thinking:364915444194344986> ${user.displayName}: You're not a part of the ${to_remove.name} group, so you cannot be removed from it.`, "info", msg);
                }
            }
        }
    },
