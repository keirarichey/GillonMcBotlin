...
module.exports = {
    ...
    execute: (msg, args, client) => {
        ...
        if (Array.from(Object.keys(COMMANDS)).includes(command)) {
            ...
            // trim undefineds before writing
            new_commands = {}
            Object.keys(COMMANDS).forEach(key => {
                if (key) {
                    new_commands[key] = COMMANDS[key]
                }
            });
            console.log(COMMANDS)
            console.log(Object.keys(COMMANDS))
            console.log(new_commands)
            console.log(Object.keys(new_commands))
            fs.writeFile("../data/commands.yml", yaml.safeDump(new_commands), (err) => {
                if (err) {
                    console.log(err);
                } else {
                    var isAre;
