        ...
        if (msg.channel == botChannel) {
            ...
            if (!Object.values(group_names).includes(msg.content)) {
                ...
                if (user.roles.has(to_add.id)) {
                    bot.sendNotification(`<:thinking:364915444194344986> ${user.displayName}: You're already part of the ${to_add.name} group, so you cannot be added to it.`, "info", msg);
                } else {
                    ...
                    bot.sendNotification(`<:white_check_mark:364924816261906432> ${user.displayName} is now a part of the ${to_add.name} group!`, "success", msg);
                }
            }
        }
    },
