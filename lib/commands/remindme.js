...
module.exports = {
	...
	execute: (msg, args) => {
		...
		con.query("INSERT INTO remindme (`messageID`, `note`, `new_date`, `origin_date`, `userID`) VALUES ?;", [[String(msg.id), note, Date.parse(newDT), Date.parse(originDT), String(msg.author.id)]], (err, rows) => {
			if (err) {
				console.log(err);
				return;
			}
