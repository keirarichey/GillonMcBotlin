var parseMessage = function(msg, args, formatMessage) {
	...
	try {
		...
		if (args.match(reg)) {
			...
			for (var interval of intervalList) {
				...
				for (var arg of argsList) {
					console.log(arg);
				}
				if (argsList.includes(interval) || argsList.includes(interval.slice(0,-1))) {
					var intervalValue	= interval;
					console.log(intervalValue + " intVal");
					var quant			= argsList.filter(arg => arg === parseInt(arg, 10));
					if (quant.length !== 1) {
...
module.exports = {
	...
	execute: (msg, args) => {
		...
		var newDT		= parseMessage(msg, args, formatMessage);
		if (newDT === null) {
			return;
		}
		else {
