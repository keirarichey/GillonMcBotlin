module.exports = {
    main: (bot, msg) => {
        ...
        if (msg.channel == botChannel) {
            ...
            var user = msg.member;
            if (!Object.values(team_names).includes(msg.content.toLowerCase())) {
                ...
                bot.sendNotification(`<@${user.id}>: \<:bt:246541254182174720> THAT WAS OUT OF BOUNDS! \`${msg.content.toLowerCase()}\` is not an accepted input!\nPlease use "+help" or "<@${bot.user.id}> help" for help.`, "error", msg);
                return;
            } else {
                var drop_array = [];
                ...
                Array.from(Object.keys(teams)).forEach(function(team_name) {
                    if (teams[team_name].includes(msg.content.toLowerCase())) {
                            new_team = team_name;
                    } else {
                        return;
                    }
