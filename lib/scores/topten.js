        ...
        var getStats = (team) => {
            ...
            .then(function(gameInfo) {
                ...
                .then(function (data) {
                    ...
                    console.log(doc.getElementsByTagName("home")[0].constructor.name);
                    console.log(doc.getElementsByTagName("away")[0].length);
                    console.log(doc.getElementsByTagName("playerid").length);

                    for (var i = 0; i < homeStats.length; i++) {
                    	playerStats = {};

                    	playerStats.id = parseInt(homeStats.getElementsByTagName("playerid")[i].childNodes[0].nodeValue);
                    	playerStats.name = homeStats.getElementsByTagName("name")[0].childNodes[0].nodeValue;
                    	playerStats.team = gameInfo.homeTeam;
                    	playerStats.number = parseInt(homeStats.getElementsByTagName("jumpernumber")[i].childNodes[0].nodeValue);
                    	playerStats.disposals = parseInt(homeStats.getElementsByTagName("kick")[i].nodeValue) + parseInt(homeStats.getElementsByTagName("handball")[i].nodeValue);
                    	playerStats.goals = parseInt(homeStats.getElementsByTagName("goal")[i].nodeValue);
                    	playerStats.behinds = parseInt(homeStats.getElementsByTagName("behind")[i].nodeValue);
                    	playerStats.dt = parseInt(homeStats.getElementsByTagName("dt")[i].nodeValue);
                    ...
                    for (var i = 0; i < awayStats.length; i++) {
                    	...
                    	playerStats.id = parseInt(awayStats.getElementsByTagName("playerid")[i].childNodes[0].nodeValue);
                    	playerStats.name = awayStats.getElementsByTagName("name")[0].childNodes[0].nodeValue;
                    	playerStats.team = gameInfo.awayTeam;
                    	playerStats.number = parseInt(awayStats.getElementsByTagName("jumpernumber")[i].childNodes[0].nodeValue);
                    	playerStats.disposals = parseInt(awayStats.getElementsByTagName("kick")[i].nodeValue) + parseInt(awayStats.getElementsByTagName("handball")[i].nodeValue);
                    	playerStats.goals = parseInt(awayStats.getElementsByTagName("goal")[i].nodeValue);
                    	playerStats.behinds = parseInt(awayStats.getElementsByTagName("behind")[i].nodeValue);
                    	playerStats.dt = parseInt(awayStats.getElementsByTagName("dt")[i].nodeValue);
