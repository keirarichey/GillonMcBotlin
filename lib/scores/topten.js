    ...
    main: (bot, msg) => {
        ...
        var getGameInfo = (team) => {
            ...
            .then(function (data) {
                ...
                allGamesRegex = /GameID=(\d+)">[^>]+>\s+(?:([A-Za-z ]+[^<]+)\s+vs[^>]+>\s*([^>]+)|([^>]+)\s+vs[^>]+>\s*([A-Za-z ]+[^<]+))/g;
            ...
            .then(function(gameInfo) {
                ...
                .then(function (data) {
                    ...
                    for (var i = 0; i < homeStats.length; i++) {
                    	...
                    	playerStats.id = parseInt(homeStats[i].getElementsByTagName("id")[0].childNodes[0].nodeValue);
                    	playerStats.name = gameInfo.homeTeam;
                    	playerStats.team = doc.getElementsByTagName("hometeam")[0].childNodes[0].nodeValue;
                    	playerStats.number = parseInt(homeStats[i].getElementsByTagName("jumpernumber")[0].childNodes[0].nodeValue);
                    	playerStats.disposals = parseInt(homeStats[i].getElementsByTagName("kick")[0].childNodes[0].nodeValue) + parseInt(homeStats[i].getElementsByTagName("handball")[0].childNodes[0].nodeValue);
                    	playerStats.goals = parseInt(homeStats[i].getElementsByTagName("goal")[0].childNodes[0].nodeValue);
                    	playerStats.behinds = parseInt(homeStats[i].getElementsByTagName("behind")[0].childNodes[0].nodeValue);
                    	playerStats.dt = parseInt(homeStats[i].getElementsByTagName("dt")[0].childNodes[0].nodeValue);
