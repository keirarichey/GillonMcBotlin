module.exports = {
	...
	execute: (msg, args) => {
		...
		var getInProgress = () => {
			...
			.then(function (data) {
				...
				if (inProgressGames.length === 0) {
					formatMessage.sendNotification("Sorry, no games are on! <:vicbias:275912832992804865>", "info", msg);
					return null;
				} else {
					inProgressIDs = [];
			...
			gameTracker.forEach( function (matchData) {
				switch (matchData.percComplete) {
					case "100":
						...
						break;
					case "75":
						matchData.status = `End of Q${matchData.currentQuarter}`;
						break;
					case "50":
		...
		.then(function(result) {
			if (result !== null) {
				formatMessage.sendNotification(result, "success", msg);
			}
		});
	}
}